...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <>
      <div 
        className="group relative bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 border border-honey-100 cursor-pointer h-full"
        onClick={() => setIsOpen(true)}
      >
        {/* Product Images Carousel - Made more compact */}
        <div className="relative aspect-square overflow-hidden">
          ...
          {/* Info icon overlay - Made smaller */}
          <div className="absolute top-2 right-2 bg-white/80 backdrop-blur-sm p-1 rounded-full opacity-70 group-hover:opacity-100 transition-opacity z-10">
            <Info size={12} className="text-honey-600" />
          </div>
        </div>
        ...
        <div className="p-3 space-y-1">
          <div className="flex justify-between items-start">
            <h3 className="font-serif font-medium text-sm text-honey-900 line-clamp-1">{product.name}</h3>
            <span className="text-honey-600 font-medium text-sm">{product.price}</span>
          </div>
          <p className="text-honey-700/80 text-xs line-clamp-2">{product.description}</p>
          
          {/* View on Amazon Button - Made more compact */}
          <a
            href={product.amazonUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-honey-800 hover:text-honey-600 transition-colors"
        ...
        {/* Buy Button (Appears on Hover) - Made more compact */}
        <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 ease-out bg-gradient-to-t from-white via-white to-transparent pt-10">
          <a
            href={product.amazonUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="block w-full bg-honey-500 hover:bg-honey-600 text-white text-center py-2 rounded-lg font-medium transition-colors shadow-md text-xs"
            onClick={(e) => e.stopPropagation()}
          >
            Buy Now
          </a>
